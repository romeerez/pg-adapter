...
const makeAuthMD5 = (
  ...
  const message = 'md5' + md5(Buffer.concat([Buffer.from(creds), salt]))
...
const makeAuthSASLContinue = (
  socket: Socket,
  request: Task,
  creds: Creds,
  data: Buffer,
) => {
  if (!request.authData) throw new Error('Invalid SASL auth flow')

  const clientNonce = request.authData.clientNonce
  if (!clientNonce) throw new Error('Invalid SASL auth flow')

  let nonce, salt, iteration
  String(getMessage(data, 0, 4))
    .split(',')
    .forEach((param) => {
      if (param[0] === 'r') nonce = param.slice(2)
      else if (param[0] === 's') salt = param.slice(2)
      else if (param[0] === 'i') iteration = param.slice(2)
    })

  if (!nonce) throw new Error('SASL: nonce missing')

  if (!salt) throw new Error('SASL: salt missing')

  if (!iteration) throw new Error('SASL: iteration missing')
  ...
  const messageWithoutProof = `c=biws,r=${nonce}`
  const authMessage = `n=*,r=${clientNonce},r=${nonce},s=${salt},i=${iteration},${messageWithoutProof}`
  ...
  request.authData.signature = createHMAC(serverKey, authMessage).toString(
    'base64',
  )

  respond(
    socket,
    Buffer.from(`p\0\0\0\0${messageWithoutProof},p=${clientProof}`),
  )
}

const makeAuthSASLFinal = (socket: Socket, request: Task, data: Buffer) => {
  const signature = request.authData?.signature
  if (!signature) throw new Error('Invalid SASL auth flow')
...
export const auth = (
  ...
  if (authType === codes.success) return
  if (authType === codes.cleartextPassword) makeAuthCleartext(socket, creds)
  else if (authType === codes.md5) makeAuthMD5(socket, creds, data)
  else if (authType === codes.SASLInit) makeAuthSASLInit(socket, request)
  ...
  else if (authType === codes.GSS) return `unsupported auth type: GSS`
  else if (authType === codes.SSPI) return `unsupported auth type: SSPI`
  else return `unknown auth type with code ${authType}`
}
